// https://code.google.com/p/infint/source/browse/trunk/InfInt.h?edit=1
...
{//PROFILED_SCOPE
    ...
    {
        if (val[(size_t)i] != rhs.val[(size_t)i])
        {
            return false;
        }
    }
...
{//PROFILED_SCOPE
    ...
    for (int i = (int) val.size() - 1; i >= 0; --i)
    {
        result = result * BASE + val[(size_t)i];
    }
...
{//PROFILED_SCOPE
    ...
    for (int i = (int) val.size() - 1; i >= 0; --i)
    {
        result = result * BASE + (unsigned int)val[(size_t)i];
    }
    return result;
}
...
{//PROFILED_SCOPE
    ...
    for (int i = (int) val.size() - 1; i >= 0; --i)
    {
        result = result * BASE + (unsigned long)val[(size_t)i];
    }
    return result;
}
...
{//PROFILED_SCOPE
    ...
    for (int i = (int) val.size() - 1; i >= 0; --i)
    {
        result = result * BASE + (unsigned long long)val[(size_t)i];
    }
    return result;
}
...
{//PROFILED_SCOPE
    ...
    if (isPositive)
    {
        for (; i >= 0; --i)
        {
            ...
            {
                int k = 0, index = i + 1;
                for (; (size_t)(index) < val.size() && val[(size_t)index] == 0; ++index)
                	++k; // count adjacent zeros on left
    ...
    else
    {
        for (; i >= 0; --i)
        {
            if (val[(size_t)i] > 0)
            {
                int k = 0, index = i + 1;
                for (; (size_t)(index) < val.size() && val[(size_t)index] == 0; ++index)
                	++k; // count adjacent zeros on right
                //if ((size_t)(index) < val.size() && val[index] < 0)
...
{//PROFILED_SCOPE
    ...
    {
        val.push_back(atoi(s.substr((size_t)i, DIGIT_COUNT).c_str()));
    }
    if (i > -DIGIT_COUNT)
    {
        std::string ss = s.substr(0, (size_t)(i + DIGIT_COUNT));
        if (ss.size() == 1 && ss[0] == '-')
        {
            pos = false;
        }
...
{//PROFILED_SCOPE
    ...
    for (int i = (int) n.val.size() - 1; i >= 0; --i)
    {
        ...
        {
            s << std::setfill('0') << std::setw(DIGIT_COUNT) << n.val[(size_t)i];
        }
    }
    return s;
}
